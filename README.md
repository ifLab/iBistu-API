 ...
 * 访问本文档提供的接口需要添加请求头，请求头内容包括：apikey，token，目前由于用户模块未完善，可仅提供apikey进行开发。
 ...
 > http://104.155.211.143/api/v2/ibistu/_table/module_map?api_key=3528bd808dde403b83b456e986ce1632d513f7a06c19f5a582058be87be0d8c2

 * 本文档提供的接口均可以通过添加可选参数`include_count=true`以获得包含数据对象总数的json数据。例如：
 > http://104.155.211.143/api/v2/ibistu/_table/module_map?include_count=true&api_key=3528bd808dde403b83b456e986ce1632d513f7a06c19f5a582058be87be0d8c2
...
## 关于模块
 * 获取iBistu相关的介绍数据
   - 接口：`http://104.155.211.143/api/v2/ibistu/_table/module_about`
   - 请求方法：get
   - 参数：无
   - 示例返回值：
  ```
...
## 班车模块
 * 获取班车数据
   - 接口：`http://104.155.211.143/api/v2/ibistu/_table/module_bus`
   - 请求方法：get
   - 参数：无
   - 示例返回值：
 ```
...
## 黄页模块
 * 获取黄页部门列表数据
   - 接口：`http://104.155.211.143/api/v2/ibistu/_table/module_department_list`
   - 请求方法：get
   - 参数：无
   - 示例返回值：
 ```
 ...
 ```
 * 获取黄页某一部门下的电话号码数据
   - 接口：`http://104.155.211.143/api/v2/ibistu/_table/module_yellowpage`
   - 请求方法：get
   - 参数：
      * `offset`：固定参数，值为`1`
      * `filter`：固定前缀`department=`，值为黄页接口1返回的数据中的`department`字段值
   - 示例：获取研究生工作办公室的电话号码：（此处参数值为：`department=10`）：`http://104.155.211.143/api/v2/ibistu/_table/module_yellowpage?offset=1&filter=department=10`
   - 示例返回值：
 ```
{
  "resource": [
...
## 地图模块
 * 获取校区位置数据
   - 接口：http://104.155.211.143/api/v2/ibistu/_table/module_map
   - 请求方法：get
   - 参数：无
   - 示例返回值：
  ```
